...
The `<Button/>` component can take 4 different styles that are passed to its `intent` prop. 😃  
...
<Canvas>
  <Story name="Disabled button">
    {() => {
      return <Button intent="accept" isDisabled>Disabled😃</Button>;
    }}
  </Story>
</Canvas>
...
⚠️ You can use the icon component provided by the library, or pass your own element. No spacing is implemented to allow more flexibility to the developer.  
...
<Canvas>
  <Story name="With link">
    {() => {
      return <Button intent="secondary" href="#with-link">With link</Button>;
    }}
  </Story>
</Canvas>
...
⚠️ When the button is loading, it will not be disabled. If you wish to implement this behavior, please use `isDisabled` as shown above.
...
<Canvas>
  <Story name="Loading">
    {() => {
      ...
      return (
        <>
          <Button intent="danger" onClick={handleClick} isLoading={isLoading} style={style}> I am loading </Button>
          <Button
            intent="accept"
            style={style} 
            onClick={handleClick} 
